...
object Game {
  ...
  def startGame(stage: Stage, mainScene: Scene, level: Int, category: String): Unit = {
    stage.scene = new Scene(800, 600) {
      ...
      val arr = generator.buildBoard(level, Categories.convert(category), 5, 10)
      val utils = new Utils
      ...
      //.............
      val wordset = generator.getWordset()
      ...
      onMouseReleased = (e) => {
        if (e.getX >= 400 - 5 * 36 && e.getX <= 400 - 5 * 36 + 359
          ...
          val intrs = wordset.intersect(wMarked)
          if (intrs.isEmpty) {
            for (cell <- signed) {
              if (!discovered.contains(cell))
                utils.dim(cell, boardCells, arr)
